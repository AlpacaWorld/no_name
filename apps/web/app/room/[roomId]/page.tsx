...
import { ROOM_EVENT, type PlayerResponse, type RoomResponse } from '@repo/contract';
...
export default function RoomLobbyPage() {
  ...
  return <main className="min-h-screen bg-slate-950 p-6 text-slate-100 sm:p-10"><section className="mx-auto max-w-2xl rounded-2xl border border-slate-700 bg-slate-900 p-6 sm:p-8"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-semibold tracking-[0.2em] text-cyan-400">WAITING ROOM</p><h1 className="mt-2 text-3xl font-bold">참가자를 기다리는 중</h1></div><span className={`rounded-full px-3 py-1 text-sm ${isConnected ? 'bg-emerald-400/15 text-emerald-300' : 'bg-amber-400/15 text-amber-300'}`}>{isConnected ? '연결됨' : '연결 중'}</span></div><div className="mt-7 rounded-xl bg-slate-950 p-4"><p className="text-sm text-slate-400">초대 링크</p><div className="mt-2 flex gap-2"><code className="min-w-0 flex-1 truncate text-sm text-cyan-300">{inviteLink}</code><button className="rounded-md border border-slate-600 px-3 py-1 text-sm hover:border-cyan-400" onClick={() => navigator.clipboard.writeText(inviteLink)}>복사</button></div></div><div className="mt-7"><h2 className="text-lg font-semibold">참가자 {room?.players.length ?? 0}명</h2><ul className="mt-3 space-y-2">{room?.players.map((player) => <li key={player.id} className="flex items-center justify-between rounded-lg border border-slate-700 px-4 py-3"><span>{player.nickname}{player.id === playerId && <span className="ml-2 text-sm text-cyan-300">나</span>}{player.isHost && <span className="ml-2 text-sm text-amber-300">방장</span>}</span><span className={`text-sm ${player.connected ? 'text-emerald-300' : 'text-slate-500'}`}>{player.connected ? '접속 중' : '재접속 대기'}</span></li>)}</ul></div>{error && <p className="mt-5 text-sm text-rose-400">{error}</p>}</section></main>;
}
